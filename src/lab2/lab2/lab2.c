...
void toBinary(int num) { 
    for (int i = 31; i >= 0; i--) { //сдвиг на i 
        putchar((num >> i) & 1 ? '1' : '0'); //тернарный оператор, ? значит если утверждение слево верно то выполняется условие после знака вопроса
        // иначе выполняется второе условие 
    }
    putchar('\n');
}
...
int main()
{
    ...
    // 1.1
    {
        ...
        //все что ни есть ноль в си истина
        y = 9; //наше число
        int flag = 0; //флаг 
        for (int i = 31; i >= 0; i--) {
            if ((y >> i) & 1) {
    ...
    int cnt = 0;
    for(int i = (sizeof(int) * 8) - 1; i >= 0; i--){ //sizeof(int) берем размер занимаемый инт в байтах
        //умножаем на 8 чтобы перевести в биты и вычитаем ноль так как счет в информатике с нуля
        if((y >> i) & 1){
            result = result + (1 << cnt);
            //
            cnt++;
    ...
    //тоже выполнение задания но за два цикла 
    for(int i = 0; i <= cnt; ++i) {
        result = result + (1 << i);
    }
    for (int i = 31; i >= 0; i--) { //пока i!=count таким образом i становится сдвигом вместо count
        putchar((result >> i) & 1 ? '1' : '0');
    }
    putchar('\n');
    putchar('\n');
}
 //запустить терминал clang (или gcc) (название файла.c) -o test && (&& - позволяют начать новую команду не ориентируясь на старую) ./test
    ...
    {
        unsigned short sNum = 0x8008; //чтобы не было уродливых единичек
        unsigned short sRes;
        //в нашей функции меняем 31 на 15 для красоты программы
        //toBinary(sNum);
        // 0x7ff7
        sRes = ~sNum;
