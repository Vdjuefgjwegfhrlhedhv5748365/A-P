...
int main()
{
    ...
    /**
     * Задание 1.1. Объявление класса. done
     *
     ...
     *
     * Ответ:
        1) вызывается неявный конструктор по умолчанию
        2) код автоматически определяет С++
        3) в этом случае код выделяет память для объекта, и но не инициализирует поля (они заполняются случайными значениями)
        4) в общем случае код выделяет память для объекта, вызывает конструкторы для полей-объектов, но не инициализирует поля встроенных типов
     ...
     *
     * Объясните ошибку компиляции, которая произошла после добавления конструктора.
     *
     * Добавьте конструктор по умолчанию, который заполняет все координаты
     * нулями.
     * Ответ: ошибка компиляции произошла, так как по умолчанию класс private и поля нельзя инициализировать
     */
     ...
     *
     * Ответ:
     1) Вызывается конструктор с 4-мя параметрами. Я явно определила этот конструктор. В этом случае конструктор создает объект rect1
     и инициализирует его поля числами (1, 2, 3, 4). В общем случае: инициализирует поля по заданным значениям.
     2) Вызывается конструктор копирования. Определяет компилятор С++. В этом случае создает объект rect2 как точную копию rect1, копирует все 4 поля.
     В общем случае: создает новый объект, копирует все поля исходного объекта, для переменных базовых типов - копирует их значения,
     для объектов полей - копирует их конструкторы копирования
     3) Вызывается конструктор копирования. Определяет компилятор С++. В этом случае создает объект rect3 как точную копию rect1, копирует все 4 поля.
     В общем случае: создает новый объект, копирует все поля исходного объекта, для переменных базовых типов - копирует их значения,
     для объектов полей - копирует их конструкторы копирования. (Строка 3 по смыслу идентична строке 2, но имеет другую синтаксическую конструкцию)
     */
     ...
     * вызывается деструктор.
     Ответ:
     1) Деструктор - метод класса; выполняет опреации обратные тем, что выполняют конструкторы.
     2) Деструктор вызывается компилятором неявно (хотя в отличие от конструктора деструктор модно вызвать явно),
     когда заканчивается время жизни объекта.
     3) В моём классе Rect есть неявный деструктор, который компилятор генерирует автоматически.
     4) Деструктор в моем случае ничего не делает, поля типа int не требуют уничтожения.
     В общем случае деструктор освобождает ресурсы, закрывает файлы, вызывает деструкторы полей-объектов.
     */
     ...
     *
     * Ответ: Конструкторы вызываются компилятором при создании объекта, деструкторы вызываются компилятором при уничтожении объекта.
     Но если вызов конструктора достаточно очевиден, то вызов деструктора определяется временем существования объекта
     */

    {
        Rect r1;                         // вызывается конструктер по умолчанию
        Rect *pR = new Rect(1, 2, 1, 2); // вызывается конструктор с параметрами
        {
            Rect r2(r1);    // вызывается конструктер копирования
            Rect arRect[2]; // вызывается конструктор по умолчанию два раза
            for (int i = 0; i < 3; i++)
            {
                static Rect r3(i, i, i, i); // конструктер с параметрами (только первая итерация = 1 раз)
                Rect r4(*pR);               // конструктер копирования (каждую итерацию = 3 раза)
                Rect r5(i, i, i, i);        // конструктор с параметрами (каждую итерацию = 3 раза)
            } // деструктер для r4, r5 (6 раз: по 2 на каждую из 3 итераций)
        } // деструктер для arRect[1], arRect[0], r2
        delete pR; // деструктер для динамического объекта
    } // деструктер для r1
    // итог: Деструкторов вызвано на один раз меньше, чем конструкторов (11 и 12 соответственно),
    //  потому что объект r3 объявлен с ключевым словом static и не уничтожается при выходе из блока, а уничтожается при завершении программы
     ...
     * обратиться к полю напрямую. Объясните ошибку компиляции.
     Ответ: Ошибка компиляции возникает, так как компилятор проверяет код на соблюдение инкапсуляции,
     прямые обращение к приватным полям объектов из вне невозможно.
     */
     ...
     * Напишите в комментариях, чем функция-член отличается от обычной функции.
     Ответ: функция-член - это метод внутри класса, который имеет прямой доступ к приватным полям,
     а обычная функция некая отдельная сущность, кторая может получить доступ к приватным полям только через публичные методы.
     Функция-член вызывается для конкретного объекта синтаксисом объект.метод(), а публичная функция принимает объект как парметр.
    ...
    Rect r9;
    r9.move(1, 2);
    std::cout << r9.getLeft() << r9.getRight() << r9.getTop() << r9.getBottom() << std::endl;
    r9.move(1);
    std::cout << r9.getLeft() << r9.getRight() << r9.getTop() << r9.getBottom() << std::endl;
    ...
    {
        Rect r1(0, 5, 0, 5), r2(0, 10, 0, 10); // вызывается конструктор с 4-мя параметрами для создания объектов r1, r2
        Rect r3 = boundingRect(r1, r2);        // 2 раза вызывается конструктор копирование, один раз вызывается конструктор с 4-мя параметрами
        std::cout << "\n"
                  << std::endl;
        printRect(r3);
        std::cout << "\n"
    ...
    {
        // посмотреть функцию getSquare()
        Rect r11(4, 12, 4, 7);
        std::cout << "Ширина: " << r11.getWidth() << "Высота: " << r11.getHeight() << "Площадь: " << r11.getSquare() << std::endl;
    ...
    {
        Barrel alch(10, 0.96); // бочка со спиртом
        Barrel water(10, 0.0); // бочка с водой

        int iteration = 0;

        std::cout << "Начальная концентрация: " << alch.getConcentration() << "%" << std::endl;

        while (alch.getConcentration() > 50)
        {
            iteration++;
            alch.Iteration(water, 0.5);
            water.Iteration(alch, 0.5);

            std::cout << "Итерация " << iteration
                      << ": концентрация = " << alch.getConcentration() << "%" << std::endl;
        }

        std::cout << "\nОтвет: концентрация стала меньше 50% на итерации " << iteration << std::endl;
    }
    ...
    {
        MyString s1("try new methods");
        s1.get(2);
        char h = 'j';
        s1.set(4, h);
        const char *str = "change string";
        s1.setNewString(str);
        s1.print();
    }
    /**
