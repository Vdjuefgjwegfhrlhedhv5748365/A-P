...
int loadCatalog_from_file(BOOK **catalog, int *cnt, int *capacity, const char *filename) {
    ...
    *cnt = atoi(buffer);
    ...
    for (int i = 0; i < *cnt; i++){
        //считываем автора
        if (fgets((*catalog)[i].author, sizeof((*catalog)[i].author), file) == NULL) break;
        (*catalog)[i].author[strcspn((*catalog)[i].author, "\n")] = '\0';
        
        //считываем название
        if (fgets((*catalog)[i].title, sizeof((*catalog)[i].title), file) == NULL) break;
        (*catalog)[i].title[strcspn((*catalog)[i].title, "\n")] = '\0';
        
        //считываем год
        if (fgets(buffer, sizeof(buffer), file) == NULL) break;
        (*catalog)[i].year = atoi(buffer);

        //считываем цену
        if (fgets(buffer, sizeof(buffer), file) == NULL) break;
        (*catalog)[i].price = atof(buffer);
        
        //считываем категорию
        if (fgets((*catalog)[i].category, sizeof((*catalog)[i].category), file) == NULL) break;
